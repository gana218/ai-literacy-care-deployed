...
> **진행 현황 범례 (2026-07-08 기준)** — 각 표의 `상태` 열과 체크박스로 구분한다.
> - ✅ **완료** — 구현·검증됨(테스트/실서버 확인)
> - 🔄 **진행/부분** — 구조는 있으나 마무리·연결 남음
> - ⬜ **미완료** — 아직 착수 안 함(주로 M3 이후·제출 단계)
>
> **한 줄 요약:** 코어 폐루프(M0·M1)와 계약·점수·통합 구조에 더해 **2번 Content Reducer real·3번 Cognitive Care real·
> 5번 QA(run_evaluation_from_state) 연결**까지 완료. 실모듈 전경로 회귀도 pytest로 박제(전체 100 passed).
> **코어 백엔드 폐루프는 사실상 완성**(§38 Definition of Done 충족).
> 남은 것은 **확장 웹/PDF 실통합·수동 E2E**와 발표 리허설.
>
...
| ✅ | M2 | 7/6 | 전 기능 통합(웹 경로) | 실제 팀원 모듈 연결(2번·3번 real·5번 QA 연결 완료, 실모듈 회귀 통과) |
...
| 상태 | 작업 | 완료 기준 | 목표일 |
|---|---|---|---|
| ✅ | **2번 Content Reducer real 연결** | `LITERACY_CONTENT_REDUCER_IMPL=real`로 실모듈 chunks/simplified/terms/difficulty가 state에 merge, 데모 정상 *(7/8 연결·키워드 폴백 검증)* | 7/6 |
| ✅ | **5번 QA 평가 연결** | `run_evaluation_from_state(state)` → `state["qa_evaluation"]`, `/finish`·`/result`에서 실행, 결과 응답 노출 *(7/8, PR #7)* | 7/8 |
| ✅ | 시계열 추세 산출 마무리 | previous_scores→trend(improving/stable/declining) 반환·그래프 데이터 제공 | 7/9 |
| 🔄 | 점수 보정 튜닝 | 가독성 정규화·교차검증 감점 실데이터 기준 조정 *(현재 공식 안정, 프리즈 앞이라 최소 조정)* | 7/9 |
| ✅ | 전 기능 통합 회귀 | content→care→routing→score→reward→profile 실모듈 경로 pytest 통과 *(test_real_path_e2e, 전체 100 passed)* | 7/9 |
...
|---|---|---|---|
| ✅ | Content adapter | 2번 Content Reducer | 실제 output을 state에 merge *(7/8 real 연결·회귀 통과)* |
...
| ✅ | Latency log | 실행 시간 측정 | trace에 latency_ms 기록(graph._run_step) |
...
| ✅ | Demo regression | 반복 실행 확인 | 실모듈 경로 회귀 pytest 박제(test_real_path_e2e) + 전체 100 passed |
...
| ✅ | QA result hook | QA 결과 연결 | 5번 run_evaluation_from_state 연결 → state["qa_evaluation"] + 결과 응답(qaEvaluation) 노출 |
...
| ✅ | 7/6(월) M2 | 코어 통합 완료 | 3번 real·2번 real 연결 · 웹 전 기능 통합 디버깅 |
| ✅ | 7/8(수) | 코어 통합 마감 | **2번 real·5번 QA 연결 완료** · 실모듈 전경로 회귀 박제(test_real_path_e2e) · QA 결과 노출 · trace 정리 (전체 100 passed) |
| ⬜ | 7/9(목) | 확장 웹/PDF 통합 | 크롬 로드 → **웹 수동 E2E 왕복** · **pdf.js 왕복 E2E** · 이벤트 스키마 정합(G6) · 온보딩/익명 UUID 연동 확인 |
...
- [x] 2번 Content Reducer 실제 출력과 연결된다. ✅ *(7/8 real 연결)*
- [x] 3번 행동 데이터 또는 focus_score와 연결된다.
- [x] 4번 프론트가 score/intervention을 표시할 수 있다.
- [x] 5번 QA가 trace를 받을 수 있다. *(run_evaluation_from_state 연결)*
...
- [x] 2번 Content Reducer 연결 ✅ *(7/8 real)*
- [x] 3번 Cognitive Care 또는 행동 데이터 연결
- [x] 4번 프론트 응답 연결
- [x] 5번 QA trace 연결 *(run_evaluation_from_state)*
- [x] 핵심 단위 테스트 작성
- [x] M1 데모 3회 이상 반복 성공
