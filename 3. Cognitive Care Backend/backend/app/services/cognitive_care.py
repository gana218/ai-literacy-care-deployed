...
def calculate_focus_score(events: List[Dict[str, Any]], baseline: Dict[str, int] = None) -> float:
    ...
    # 7/11 수정: 스크롤 이벤트가 매우 빈번하게(150ms마다) 발생하므로,
    # 윈도우 크기가 너무 작으면(12개=1.8초) 감점 이력(blur 등)이 순식간에 사라져 퀴즈 트리거(30점 미만)가 불가능했습니다.
    # 윈도우를 60개(약 9초 분량)로 늘려 감점이 누적될 수 있도록 조정합니다.
    recent = events[-60:]
    score = 100.0
