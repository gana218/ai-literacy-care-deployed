...
def fallback_restructured_text(original_text: str) -> str:
    """
    LLM 재구성 실패 시 원문 텍스트를 그대로 반환한다.
    """
...
def fallback_quiz(chunk_id: str) -> QuizDict:
    """
    퀴즈 생성 실패 시 반환할 기본 O/X 퀴즈.
    사전에 준비된 일반 독해력 확인 문제를 반환한다.
    """
    return QuizDict(
        chunk_id=chunk_id,
        question="방금 읽은 문단은 특정 개념이나 현상에 대해 설명하고 있다.",
        options=["O", "X"],
        correct_option=1,
        explanation="이 퀴즈는 기본 독해력 확인 문항입니다. 대부분의 비문학 문단은 특정 개념이나 현상을 설명하는 구조를 가지므로, 본문을 주의 깊게 읽었다면 판별할 수 있습니다.",
    )
